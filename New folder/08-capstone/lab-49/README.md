## Lab 49 – Capstone: Multi-Stage Intrusion Across On-Prem and Cloud (Advanced)

### Scenario
You are part of a centralised SOC that monitors both on‑premises infrastructure and cloud workloads for a global organisation. Over several days, multiple low‑to‑medium severity alerts have fired across different tools: phishing emails, unusual VPN logins, suspicious cloud console activity, and endpoint detections. A high‑severity alert has now been raised for data access from a privileged cloud storage account. You must piece together events from across the stack to determine what happened and coordinate a response.
